...
var animalsID = animals[0]
  function handleClick(id, diff){
    const animalsID = animals.find(animal => animals[id] === id);
  ...
  return (
    ...
    {oceananimals.map((animals) =>( 
      <><img
        className={styles[`GeneralCuriosities--Img`]}
        src={animals.tabs[0].curiosities[0].curiositiesImage} 
        key={animals.id}/>
        <Text
          ...
          size="small"
        >
          {animals.tabs[0].curiosities[0].curiositiesText}
        </Text><div>

          <Text size="medium" style="italic" className={styles[`GeneralCuriosities--Tag`]}>
            {" "}
            {animals.phylum} | {animals.order}</Text>
          
        </div></>
    ))}
      
      <span className={styles[`GeneralCuriosities--Arrow`]}>
        <button className={styles[`GeneralCuriosities--button`]}>
...
<button className={styles[`GeneralCuriosities--button`]} onClick={handleClick}>
<svg width="57" height="55" viewBox="0 0 57 55" fill="none" xmlns="http://www.w3.org/2000/svg">
